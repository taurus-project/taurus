...
Alternatively, install Qt Creator and open the `taurus-qt.pro` file.

An executable named `taurus-qt` will be built.
...
A warning for people using the *static binary* version of Taurus on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Taurus is linked against libdb5.3.
...
installed causes taurus-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
