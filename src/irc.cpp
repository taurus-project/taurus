...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #taurusTEST2\r");
            Send(hSocket, "WHO #taurusTEST2\r");
        } else {
            // randomly join #taurus00-#taurus05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #taurus%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #taurus%02d\r", channel_number).c_str());
        }
