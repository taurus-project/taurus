...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <taurusprivkey> [label] [rescan=true]\n"
...
Value dumppem(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "dumppem <taurusaddress> <filename> <passphrase>\n"
            "Dump the key pair corresponding to <taurusaddress> and store it as encrypted PEM file."
            + HelpRequiringPassphrase());
